...
import { Ok, Result, Some } from "@chocolatelib/result";
import { StateError, StateNumber, StateNumberLimits } from "../../src"

describe('Initial state', function () {
    it('Creating a state with initial value', async function () {
        let state = new StateNumber(Ok(2));
...
describe('Setting state value', function () {
    ...
    it('From user context with standard setter function', async function () {
        let state = new StateNumber(Ok(2), true);
        expect((await state).unwrap).equal(2);
        state.set(Ok(4))
        expect((await state).unwrap).equal(4);
    });
    it('From user context with custom function', async function () {
        let state = new StateNumber(Ok(2), val => Some(Ok(val * 2)));
        expect((await state).unwrap).equal(2);
        state.write(4)
...
describe('Value subscriber', function () {
    ...
    it('Add two subscribers with update set true', async function () {
        ...
        let values = await Promise.all([
            new Promise<Result<number, StateError>>((a) => { state.subscribe(a, true) }),
            new Promise<Result<number, StateError>>((a) => { state.subscribe(a, true) }),
        ])
    ...
    it('Setting value with multiple subscribers', async function () {
        let state = new StateNumber(Ok(2));
        let sum = 0
        state.subscribe((val) => { sum += val.unwrap }, true)
        state.subscribe((val) => { sum += val.unwrap }, true)
        state.subscribe((val) => { sum += val.unwrap }, true)
        state.set(Ok(10));
        expect(sum).equal(36);
...
describe('Number limits', function () {
    it('Min max', async function () {
        let state = new StateNumber(Ok(2), true, new StateNumberLimits(5, 54));
        expect((await state).unwrap).equal(2);
        state.write(1)
    ...
    it('Step sizes', async function () {
        let state = new StateNumber(Ok(2), true, new StateNumberLimits(undefined, undefined, 0.22));
        expect((await state).unwrap).equal(2);
        state.write(4)
        expect((await state).unwrap).equal(3.96);
    });
    it('Step sizes and offset', async function () {
        let state = new StateNumber(Ok(2), true, new StateNumberLimits(undefined, undefined, 0.22, 0.12));
        expect((await state).unwrap).equal(2);
        state.write(4)
        expect((await state).unwrap).equal(4.08);
    });
    it('Step sizes and offset and min max', async function () {
        let state = new StateNumber(Ok(2), true, new StateNumberLimits(8, 77, 0.22, 0.12));
        expect((await state).unwrap).equal(2);
        state.write(4)
        expect((await state).unwrap).equal(8);
        state.write(90)
