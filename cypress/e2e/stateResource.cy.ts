...
import { StateResource, StateResourceFunc } from "../../src";
...
describe('Getting state value', async function () {
    ...
    it('Using then with chaining return', function (done) {
        let state = new StateResourceFunc<number>(async () => { return Ok(2); }, () => { throw new Error('This should not be called'); }, () => { throw new Error('This should not be called'); }, 50, 50, 50);
        state.then((val) => {
...
describe('Async subscribe', function () {
    ...
    it('Async unsubscribe', async function () {
        ...
        let state = new StateResourceFunc<number>(
            ...
            , () => { calls[2](0); }
            , 50, 50, 50);
        let func = state.subscribe(() => { calls[1](0); }, true);
