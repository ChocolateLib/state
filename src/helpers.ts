...
export class StateStringLimits implements StateLimiter<string> {
  ...
  constructor(maxLength?: number, maxByteLength?: number) {
    ...
    if (maxByteLength !== undefined) this.maxByteLength = maxByteLength;
  }
  ...
  check(value: string): string | undefined {
    ...
    if (
      "maxByteLength" in this &&
      new TextEncoder().encode(value).length > <number>this.maxByteLength
    )
      return (
        "the text is longer in bytes than the limit of " + this.maxByteLength
      );
    return undefined;
  }
  ...
  limit(value: string): Option<string> {
    ...
    if (this.maxByteLength) {
      value = new TextDecoder().decode(
        new TextEncoder().encode(value).slice(0, this.maxByteLength)
      );
